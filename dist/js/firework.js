...
Emitter.prototype.reset = function () {
  ...
  let color = [~~(Math.random() * 150) + 10, ~~(Math.random() * 150) + 10,
  ~~(Math.random() * 150) + 10]
  this.particles = [];
...
Emitter.prototype.render = function (imgData) {
  ...
  for (i = 0; i < this.particles.length; i++) {
    let particle = this.particles[i],
      dist = Math.sqrt((particle.x - particle.ox) * (particle.x - particle.ox)
    ...
    if (particle.render && particle.xPos < width && particle.xPos
      > 0 && particle.yPos > 0 && particle.yPos < height) {
...
function render(currentTime) {
  ...
  angleX = Math.sin(angle + 111);
  angleZ = Math.sin(angle + 111);
...
function colorCycle() {
  ...
  colors.r = ~~(Math.sin(.3 * cycle + 10) * 127 + 128);
  colors.g = ~~(Math.sin(.3 * cycle + 12) * 127 + 128);
  colors.b = ~~(Math.sin(.3 * cycle + 14) * 127 + 128);
}
